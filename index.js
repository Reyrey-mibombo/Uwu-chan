...
client.once('ready', async () => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder()
            .setName('setup_apply')
            .setDescription('Setup apply button in a channel (Owner only)')
        ...
        new SlashCommandBuilder()
            .setName('set_logs')
            .setDescription('Set log channel for applications (Owner only)')
            .addChannelOption(option => 
                option.setName('channel')
    ...
    client.user.setPresence({
        activities: [{
            name: '/apply for staff | 7 questions',
...
async function handleCommand(interaction) {
    if (interaction.commandName === 'apply') {
    ...
    else if (interaction.commandName === 'set_logs') {
    ...
    else if (interaction.commandName === 'view_apps') {
        if (interaction.user.id !== OWNER_ID) {
            return interaction.reply({ content: '❌ Owner only!', ephemeral: true });
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { name: '**👑 Owner Commands**', value: '━━━━━━━━━━━━━━━━', inline: false },
    ...
    else if (interaction.commandName === 'setup_apply') {
        if (interaction.user.id !== OWNER_ID) {
            return interaction.reply({ content: '❌ Owner only!', ephemeral: true });
        }
        const channel = interaction.options.getChannel('channel');
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '🎯 Pic Perms', value: `Put \`${STATUS_TRIGGER}\` in status for special role`, inline: true },
                { name: '📊 Positions', value: '11 positions available', inline: true }
            );
...
if (!TOKEN || !CLIENT_ID) {
    ...
    console.error('Please set DISCORD_TOKEN and CLIENT_ID in Railway variables.');
    process.exit(1);
}
